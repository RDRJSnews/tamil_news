...
def main(lang_code):
    ...
    try:
        log_print("INFO", "Calling news_text_main to generate news content")
        news_text = news_text_main(lang)
        
        if not news_text or news_text.startswith("An error occurred"):
            log_print("ERROR", "Failed to get news text from news_text_main")
            raise Exception("News text generation failed")
        
        log_print("INFO", "News text generated successfully")
