...
logger.info("Configuring Gemini API with environment variable")
...
def list_available_models():
    ...
    try:
        for m in genai.list_models():
            logger.debug(f"Model: {m.name}")
            logger.debug(f"Display name: {m.display_name}")
            logger.debug(f"Description: {m.description}")
            logger.debug("---")
        logger.info("Successfully listed available models")
...
def setup_model():
    ...
    logger.info("Setting up Gemini model with optimized parameters")
    generation_config = {
    ...
    logger.info("Gemini model setup completed successfully")
    return model

def format_response(text):
    """Format the response text for better readability."""
    logger.debug("Starting text formatting process")
    ...
    for i, line in enumerate(lines):
        ...
        if ('இது போல தினசரி செய்திகள்' in line or 
            ...
            formatted_lines.append(line)
            logger.debug(f"Added social media line: {line[:50]}...")
            continue
        ...
        if ':' in line and not line.strip().endswith(':'):
            ...
            if len(parts) == 2:
                ...
                logger.debug(f"Formatted news line {i+1}: {title.strip()[:30]}...")
            else:
                formatted_lines.append(line)
                logger.debug(f"Added unformatted line {i+1}: {line[:50]}...")
        else:
            formatted_lines.append(line)
            logger.debug(f"Added simple line {i+1}: {line[:50]}...")
    
    formatted_text = '\n'.join(formatted_lines)
    logger.info(f"Text formatting completed. Final length: {len(formatted_text)} characters")
    return formatted_text

def get_gemini_response(prompt):
    """Get response from Gemini API for the given prompt."""
    logger.info("Initiating Gemini API request")
    ...
    try:
        ...
        logger.debug(f"Raw response length: {len(response.text)} characters")
...
def main(lang):
    logger.info("=== Starting News Text Generation Process ===")
    ...
    logger.info("Generating news content with Gemini AI...")
    response = get_gemini_response(prompt)
    
    if response.startswith("An error occurred"):
        logger.error("Failed to generate news content")
        return response
    
    logger.info("=== News Text Generation Completed Successfully ===")
    return response
